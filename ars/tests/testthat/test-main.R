...
test_that("special case are correct", {
  ...
  expect_true(ks.test(ars(dchisq_df3, c(0,Inf), n = 10000), pchisq_df3)$p.value > 0.05)
  ...
  expect_true(ks.test(ars(dtst_beta, c(0,1), n = 10000), ptst_beta)$p.value > 0.05)
  ...
  expect_true(ks.test(ars(dtrctNmT, c(-1,1), n = 10000), ptrctNmT)$p.value > 0.05)
...
test_that("throw error when density is not concave", {
  ...
  expect_error(ars(dtst_beta, c(0,1), n = 10000), "density function is not concave")
  
  # test student t
  dtst_t <- function(x) { return( dt(x, df = 2) ) }
  expect_error(ars(dtst_t, c(-Inf,Inf), n = 10000), "density function is not concave")
  
  # test F
  dtst_f <- function(x) { return( df(x, 3, 3) ) }
  expect_error(ars(dtst_f, c(0,Inf), n = 10000), "density function is not concave")
})
